...
`,n=n.substring(g.length+1),c=!0),!c){let b=this.rules.other.nextBulletRegex(k),P=this.rules.other.hrRegex(k),w=this.rules.other.fencesBeginRegex(k),te=this.rules.other.headingBeginRegex(k),ye=this.rules.other.htmlBeginRegex(k),Se=this.rules.other.blockquoteBeginRegex(k);for(;n;){let G=n.split(`
`,1)[0],z;if(g=G,this.options.pedantic?(g=g.replace(this.rules.other.listReplaceNesting,"  "),z=g):z=g.replace(this.rules.other.tabCharGlobal,"    "),w.test(g)||te.test(g)||ye.test(g)||Se.test(g)||b.test(g)||P.test(g))break;if(z.search(this.rules.other.nonSpaceChar)>=k||!g.trim())p+=`
