...
// 開いた時点でアシスタントの注釈が表示されている状態にする
const INTRO_MESSAGE: ChatMessage = {
	id: 0,
	role: "assistant",
	text: "私は製作者のRAGチャット秘書です。\n経歴・思考・制作物についてのご質問にお答えします。お気軽にどうぞ。\n1人あたり2問/2日までの制限があります。\n お答えしづらいご質問には、秘書として回答を控えさせていただく場合がございます。",
};
	...
	return (
		...
		>
			...
			{item.sources && item.sources.length > 0 ? (
				<ul className="editorTranscriptSources">
...
const RagChat = ({ className }: RagChatProps) => {
	...
	return (
		<div className={`editorOverlayInner [--wid:960px] ${className ?? ""}`}>
			...
			<section className="editorDock w-full rounded-[38px] border border-TC/10 bg-WH/70 px-5 py-2 text-TC shadow-[0_22px_72px_var(--TC10)] backdrop-blur-[28px]">
				<form
				...
				>
					<div className="editorComposerField relative flex-1 min-w-0">
						<textarea
							aria-label="質問を入力"
							ref={textareaRef}
							className="editorComposerInput block w-full resize-none overflow-hidden border-0 bg-transparent px-1 py-2 outline-none"
					...
					<button
						aria-label="送信"
						className="editorSendButton inline-flex size-10 shrink-0 items-center justify-center rounded-full bg-MC text-WH transition  disabled:cursor-progress disabled:opacity-65"
						disabled={!canSubmit}
						title="送信"
						type="submit"
					>
