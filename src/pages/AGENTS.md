...
## 今回のページ作成について (監督エージェントに伝えた文面)

/Users/yanoseiji/projects/0413portfolio/public/images/lumaport
/Users/yanoseiji/projects/0413portfolio/src/pages/Lumaport.tsx
ページの土台と最低限必要な画像素材を作ってあるので、ページを作成してください。
- セクションごとに作業単位を分けて、Pagesディレクトリ直下にページ名のディレクトリを作成し、その中にコンポーネントやモジュールをまとめてください。
- セクションごとに別のサブエージェントにスポーンしてください。
- ヒーローセクションとヘッダーは同じサブエージェントに担当させてください。
- ヘッダーのリンクとロゴや全セクションのボタンはすべてReact Router DOM でダミーリンクを設定してください。
- lumaport/hero/bg01.png この画像がポジションフィックスでどのセクションにも背景で表示されることをヒーローセクション担当に伝えてください。/lumaport/hero/mv01.png ヒーローセクションの背景画像はこれを使います。hero担当は作業が多いのでポジションフィックスはまだ作らないでいいです。
- 各エージェントに必ずこれを読むように伝えてください。/Users/yanoseiji/projects/0413portfolio/src/pages/AGENTS.md
- 画像素材作成に失敗している箇所がありますがデザインと違うと思っても用意されている素材で仮の状態で構築を進めてください。
- その他のテキストや背景色はすべて DOM構造で構築します。アイコンフォントを使う場合は必ずphospherIconsを使用してくださいすぐに見つからない場合は仮のアイコンでOKです
